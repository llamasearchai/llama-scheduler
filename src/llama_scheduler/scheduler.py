        ...
        for job_def in jobs:
            try:
                ...
                if not all([job_id, func_path, trigger_type]):
                    logger.error(
                        f"Skipping job due to missing id, func, or trigger: {job_def}"
                    )
                    continue
                ...
                logger.error(
                    f"Failed to load job definition {job_def}: {e}", exc_info=True
                )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(default_config):
        logger.warning(
            f"Config file '{default_config}' not found, creating dummy config."
        )
